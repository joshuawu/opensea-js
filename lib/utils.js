...
var types_1 = require("wyvern-schemas/dist-tsc/types");
...
exports.ENJIN_ADDRESS = '0x8562c38485B1E8cCd82E44F89823dA76C98eb0Ab';
...
exports.findAsset = function (web3, _a, retries) {
    var account = _a.account, proxy = _a.proxy, wyAsset = _a.wyAsset, schema = _a.schema;
    if (retries === void 0) { retries = 1; }
    return __awaiter(_this, void 0, void 0, function () {
        function _handleOtherOwner(o) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (o && o != '0x') {
                                return [2 /*return*/, types_2.WyvernAssetLocation.Other];
                            }
                            if (retries <= 0) {
                                return [2 /*return*/, undefined];
                            }
                        ...
                        case 1:
                            _a.sent();
                            return [2 /*return*/, exports.findAsset(web3, { account: account, proxy: proxy, wyAsset: wyAsset, schema: schema }, retries - 1)];
                    }
                });
            });
        }
        var owner, proxyCount, myCount, ownerOf, countOf, abi_1, contract_1, inputs_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    inputs_1 = abi_1.inputs.map(function (i) { return i.value.toString(); });
                    return [4 /*yield*/, promisify(function (c) {
                            var _a;
                            return (_a = contract_1[abi_1.name]).call.apply(_a, inputs_1.concat([c]));
                        })];
                case 1:
                    owner = _b.sent();
                ...
                case 5:
                    if (owner !== undefined) {
                        if (proxy && owner.toLowerCase() === proxy.toLowerCase()) {
                            return [2 /*return*/, types_2.WyvernAssetLocation.Proxy];
                        }
                        else if (owner.toLowerCase() === account.toLowerCase()) {
                            return [2 /*return*/, types_2.WyvernAssetLocation.Account];
                        }
                        else {
                            return [2 /*return*/, _handleOtherOwner(owner)];
                        }
                    }
...
function getWyvernAsset(schema, asset, quantity) {
    ...
    if ('identifier' in asset) {
        var classID = asset.tokenId != null
            ? asset.tokenId
            : undefined;
        return getWyvernFTAsset(schema, asset.tokenAddress, asset.identifier, quantity, classID);
    }
    else {
        return getWyvernNFTAsset(schema, asset.tokenId, asset.tokenAddress);
    }
}
 ...
 */
function getWyvernNFTAsset(schema, tokenId, tokenAddress) {
 ...
 */
function getWyvernFTAsset(schema, address, identifier, quantity, classID) {
    var fromWySchema = schema.assetFromFields({
        'ID': classID,
        'Quantity': quantity,
        'Address': address.toLowerCase(),
    });
    return __assign({}, fromWySchema, { identifier: identifier });
}
 ...
 */
function getWyvernENSNameAsset(schema, name) {
...
exports.getWyvernENSNameAsset = getWyvernENSNameAsset;
/**
...
function getWyvernBundle(schema, assets) {
    var wyAssets = assets.map(function (asset) { return getWyvernNFTAsset(schema, asset.tokenId, asset.tokenAddress); });
